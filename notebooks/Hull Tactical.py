...
if MODEL_SRC.exists():
    shutil.copy(MODEL_SRC, Path("hull_modeling.py"))
    print(f"hull_modeling.py materializado em {Path('hull_modeling.py').resolve()}")
else:
    raise FileNotFoundError("src/hull_modeling.py não encontrado; garanta que o repo está completo.")
...
hm.MARKET_COL = MARKET_COL
hm.RF_COL = RF_COL
hm.IS_SCORED_COL = IS_SCORED_COL
