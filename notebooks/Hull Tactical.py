...
KAGGLE_USERNAME = "marcux777"  # seu user Kaggle
KAGGLE_KEY = "c66fae692bb982ce368a2c82456e342b"       # sua key Kaggle
...
def evaluate_baselines(train_df, feature_cols, target_col):
    ...
    print(f"Baseline constante (alloc=1): Sharpe_adj={sharpe_const:.4f} | vol={const_details.get('strategy_vol', float('nan')):.2f}")
    print(
        f"Linear Ridge: Sharpe_adj={sharpe_lin:.4f} | best_alpha={best_alpha_lin:.2f} | best_k={best_k_lin:.2f} | vol={lin_details.get('strategy_vol', float('nan')):.2f} | RMSE={rmse_lin:.6f}"
    )
    return {
        "const": {"sharpe": sharpe_const},
        "ridge": {"sharpe": sharpe_lin, "rmse": rmse_lin, "best_k": best_k_lin, "best_alpha": best_alpha_lin},
    }
...
def constant_allocation_cv(df, n_splits=5, val_frac=0.1):
    ...
    for i, (mask_tr, mask_val) in enumerate(splits, 1):
        ...
        print(f"Constante fold {i}: Sharpe_adj={sharpe_const:.4f} | vol={details.get('strategy_vol', float('nan')):.2f} | n_scored={n_scored}")
    if metrics:
        print(f"Sharpe médio baseline constante: {np.mean([m['sharpe'] for m in metrics]):.4f}")
    return metrics
...
def time_cv_lightgbm_fitref(
    ...
    for i, (mask_tr, mask_val) in enumerate(splits, 1):
        ...
        print(
            f"[fit_ref] Fold {i}: Sharpe_adj={sharpe_val:.4f} | alpha={best_alpha:.2f} | k={best_k:.2f} | n_scored={metrics[-1]['n_scored']}"
        )
    if metrics:
        print(f"[fit_ref] Sharpe médio: {np.mean([m['sharpe'] for m in metrics]):.4f}")
    return metrics
...
def make_time_splits(df, date_col="date_id", n_splits=5, val_frac=0.1, min_train_frac=0.5):
    """
    Splits temporais por blocos contíguos de datas:
    - janela de treino que se expande;
    - bloco de validação com tamanho ~= val_frac das datas;
    - tenta balancear a quantidade de linhas is_scored==1 por fold quando disponível;
    - ignora folds sem linhas is_scored==1 (quando existir).
    """
    ...
    if "is_scored" in df.columns:
        ...
        for _ in range(n_splits):
            ...
            if end <= idx:  # fallback para pelo menos um bloco
...
def adjusted_sharpe_score(
    ...
    """
    Replica a métrica oficial (volatility-adjusted / modified Sharpe) aplicada apenas nas linhas is_scored==1.
    Retorna (score, details).
    Referência direta: `modified_sharpe_ratio` do script de avaliação oficial da competição (página de Evaluation / notebook benchmark público no Kaggle).
    """
...
# %% [markdown]
# ### Micro unit tests da métrica
# Checks rápidos com dados sintéticos para manter a métrica alinhada ao script oficial (caso de alocação constante bate o valor de referência).

# %%
def _test_adjusted_sharpe_score():
    ...
    alloc_const = pd.Series([1.0, 1.0, 1.0, 1.0], index=df_synth.index)
    score_const, _ = adjusted_sharpe_score(
        df_synth, alloc_const, market_col="forward_returns", rf_col="risk_free_rate", is_scored_col="is_scored"
    )
    assert np.isclose(score_const, 7.515351847771772, atol=1e-9), "Alocação constante deve bater o valor de referência do script oficial."

    df_masked = df_synth.copy()
    df_masked.loc[df_masked.index[-1], "is_scored"] = 0
    score_masked, _ = adjusted_sharpe_score(
        df_masked, alloc_const, market_col="forward_returns", rf_col="risk_free_rate", is_scored_col="is_scored"
    )
    assert np.isclose(score_masked, 5.477617347335218, atol=1e-9), "Linhas não pontuadas precisam ser ignoradas no cálculo."

    alloc_extreme = pd.Series([5.0, -3.0, 4.0, 10.0], index=df_synth.index)
    score_clip, _ = adjusted_sharpe_score(
        df_synth, alloc_extreme, market_col="forward_returns", rf_col="risk_free_rate", is_scored_col="is_scored", clip_alloc=True
    )
    assert np.isclose(score_clip, 14.022075320006172, atol=1e-9), "Clipping 0-2 deve reproduzir o Sharpe esperado para posições truncadas."


_test_adjusted_sharpe_score()
...
def time_cv_lightgbm(
    ...
    for i, (mask_tr, mask_val) in enumerate(splits, 1):
        ...
        best_k, best_alpha, best_score = optimize_allocation_scale(
        ...
        if pd.isna(sharpe_adj) or sharpe_adj == -np.inf:
            print(f"Fold {i}: sem score válido (possivelmente sem linhas is_scored==1); pulando.")
            continue
        ...
        print(
            f"{prefix}Fold {i}: Sharpe_adj={sharpe_adj:.4f} | alpha={best_alpha:.2f} | k={best_k:.2f} | vol={details.get('strategy_vol', float('nan')):.2f} | const={const_sharpe:.4f} | iter={best_iter} | n_val={n_val} | n_scored={n_scored}"
        )

    if metrics:
        avg_sharpe = np.mean([m["sharpe"] for m in metrics])
        avg_const = np.mean([m.get("const_sharpe", 0) for m in metrics])
        print(f"{prefix}Média Sharpe (folds): {avg_sharpe:.4f} | baseline const: {avg_const:.4f}")
    return metrics
...
def sanity_shuffle_test(df, feature_cols, target_col, n_splits=3):
    ...
    if metrics:
        ...
        print(f"Sanity (alvo embaralhado): Sharpe médio={mean_sharpe:.4f}  (esperado ~0)")
    return metrics
...
def clipping_sensitivity(pred_returns, df_eval, k=None, alpha=1.0):
    ...
    print(f"Sharpe com clipping={sharpe_clip:.4f} | sem clipping={sharpe_no_clip:.4f}")
...
def stability_check(df, feature_cols, target_col, configs=((5, 0.1), (4, 0.15))):
    ...
    for n_splits, val_frac in configs:
        ...
        if metrics:
            ...
            print(
                f"Stability CV n_splits={n_splits}, val_frac={val_frac:.2f}: "
                f"Sharpe_mean={summary['sharpe_mean']:.4f} | std={summary['sharpe_std']:.4f} | "
                f"k_mediana={summary['k_median']:.2f} | n_scored min/med/max={summary['n_scored_min']}/"
                f"{summary['n_scored_med']}/{summary['n_scored_max']} | const_mean={summary['const_sharpe_mean']:.4f}"
            )
    return results
...
def run_cv_preds(
    ...
    for i, (mask_tr, mask_val) in enumerate(splits, 1):
        ...
        if model_kind == "lgb":
            ...
            print(f"Modelo {model_kind} não suportado ou dependência ausente.")
            continue
...
def calibrate_k_from_cv_preds(pred_df, k_grid=None, intercept=1.0):
    ...
    if pred_df is None or pred_df.empty or "pred_return" not in pred_df.columns:
        print("Sem predições OOF com pred_return para calibrar k.")
    ...
    if res_df.empty:
        print("Grid de k não retornou Sharpe válido.")
        return res_df, None
    best_row = res_df.iloc[0]
    best_k = float(best_row["k"])
    print(f"Melhor k (CV preds, intercept={intercept}): {best_k:.2f} | Sharpe_adj={best_row['sharpe']:.4f}")
    return res_df, best_k


# %% [markdown]
# ### Calibração leve de clipping/tanh/z-score das features intencionais
